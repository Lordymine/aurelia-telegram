...
describe('ClaudeCodeBridge', () => {
  ...
  describe('execute', () => {
    ...
    it('should pass allowedTools when provided', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith(
        'claude',
        ['--print', '--verbose', '--output-format', 'stream-json', '--dangerously-skip-permissions', '--allowedTools', 'Read,Write'],
        expect.any(Object),
      );
    });
    ...
    it('should pass appendSystemPrompt when provided', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith(
        'claude',
        ['--print', '--verbose', '--output-format', 'stream-json', '--dangerously-skip-permissions', '--append-system-prompt', 'extra context'],
        expect.any(Object),
      );
    });
